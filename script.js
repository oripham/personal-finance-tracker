...
document.querySelector('[data-tab="equal"]').addEventListener("click", loadCategoriesIntoDropdown);

function getExpensesByCategory() {
    let expenses = {};
    document.querySelectorAll("#expense-table-body tr").forEach(row => {
        let category = row.children[2].textContent.trim();
        let amount = parseFloat(row.children[1].textContent.replace(/\D/g, "")) || 0;
...
document.getElementById("compare-category").addEventListener("change", updateTimeComparisonChart);
document.getElementById("time-mode").addEventListener("change", updateTimeComparisonChart);
